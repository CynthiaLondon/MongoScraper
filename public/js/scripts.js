$(document).ready(function() {
    ...
    $(document).on("click", ".btn-save", function(e){
      ...
      // Get the id of the data attribute from parent div
      var articleId = $(this).parents('.article-item').data("id");
  
      // Ajax PUT request for save route
      $.ajax({
        method: "PUT",
        url: "/save/" + articleId,
      }).then(function(data) {
        if (data) {
          Materialize.toast('Yay! Article added to Article Library!', 3000);
    ...
    $(document).on("click", ".btn-unsave", function(e){
      ...
      // Get the id of data attribute from parent div
      var articleId = $(this).parents('.article-item').data("id");
  
      // Ajax PUT request forsave route
      $.ajax({
        method: "PUT",
        url: "/unsave/" + articleId,
      }).then(function(data) {
        if (data) {
          Materialize.toast('Boo! Article Removed from Article Library.', 3000);
          $("[data-id='" + data._id + "']").remove(); 
          $grid.masonry();
        }
      });
    ...
    $(document).on("click", "#save-comment", function(e){
      ...
      // Ajax PUT request for comments route
      $.ajax({
        method: "POST",
        ...
        if (data) {
          Materialize.toast('Comment is saved.', 3000);
          $("#comment").val('');
          $('.modal').modal('close');
        }
      });
    ...
    $(document).on("click", "#remove-comment", function(e){
      ...
      // Ajax PUT request for removing comments
      $.ajax({
        method: "PUT",
        ...
        if (data) {
          Materialize.toast('Comment is removed.', 3000);
          $('.modal').modal('close');
        }
      });
